...
function FreelancerSearchContent() {
  ...
  const filteredServices = services.filter((service) => {
    ...
    if (selectedLocation !== "Any location") {
      ...
      const normalizedSelection = selectedLocation
        .toLowerCase()
        .replace("united kingdom", "uk")
        .replace("united states", "usa");

      if (
        !normalizedLocation.includes(normalizedSelection) &&
        !(normalizedSelection === "usa" && normalizedLocation.includes("us")) &&
        !(
          normalizedSelection === "europe" &&
          ["germany", "spain", "france", "italy", "russia", "uk"].some(
    ...
    // Rating filter
    const anyRatingFilterSelected = Object.values(ratingFilters).some(
      (value) => value,
    );
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg font-medium">
                  {isLoading
                    ? "Loading..."
                    : `${pagination?.total_services || matchingCount} freelancers available`}
                </h2>
                <SortingOptions sortOption={sortBy} setSortOption={setSortBy} />
              </div>
