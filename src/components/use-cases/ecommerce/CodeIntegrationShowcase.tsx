...
await oh.escrows.fund(order.id, {
  ...
  },
  {
    ...
    lang: "javascript",
    icon: Monitor,
    description: "Delivery confirmation from the buyer — triggers on-chain fund release.",
    docHref:
...
function CopyButton({ code }: { code: string }) {
  ...
  return (
    <button
      ...
      style={
        copied
          ? { background: "var(--color-primary)", boxShadow: "0 4px 12px rgba(20,154,155,0.4)" }
          : { background: "rgba(255,255,255,0.06)", backdropFilter: "blur(4px)" }
      }
    >
      <span className="flex items-center gap-1.5">
        {copied ? <Check size={12} className="stroke-[3]" /> : <Copy size={12} className="stroke-[2.5]" />}
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-12 text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-content-secondary">
            ...
            handles order creation, escrow funding, and delivery confirmation in three
            explicit calls — no manual fund management required.
          </p>
        </div>
          ...
          {[
            {
              ...
              detail: "Creates an escrow-safe purchase with a delivery window and atomic refund path.",
            },
            {
