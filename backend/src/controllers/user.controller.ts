...
export const createUserHandler = async (
  ...
  try {
    const { wallet_address, username, name, bio, email, is_freelancer } =
      req.body;
...
export const getUserByIdHandler = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(buildSuccessResponse(user, "User fetched successfully"));
  } catch (error: any) {
    // Handle Supabase errors
    if (error.code && error.message) {
      throw mapSupabaseError(error);
...
export const updateUserHandler = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(buildSuccessResponse(changedFields, "User updated successfully"));
  } catch (error: any) {
    // Handle Supabase errors
    if (error.code && error.message) {
      throw mapSupabaseError(error);
...
export const getAllUsersHandler = async (
  ...
  try {
    const filters: UserFilters = {
      ...
      is_freelancer:
        req.query.is_freelancer !== undefined
    ...
    res.status(200).json(
      buildPaginatedResponse(result.users, "Users retrieved successfully", {
