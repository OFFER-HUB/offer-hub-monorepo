...
export const createServiceHandler = async (
  ...
  res
    .status(201)
    .json(buildSuccessResponse(newService, "Service created successfully"));
};
...
export const getAllServicesHandler = async (
  ...
  const filters: ServiceFilters = {
    ...
    min_price: req.query.min ? parseFloat(req.query.min as string) : undefined,
    max_price: req.query.max ? parseFloat(req.query.max as string) : undefined,
  ...
  if (filters.page && filters.page < 1) {
    res
      .status(400)
      .json(buildErrorResponse("Page number must be greater than 0"));
    return;
  }

  if (filters.limit && (filters.limit < 1 || filters.limit > 50)) {
    res.status(400).json(buildErrorResponse("Limit must be between 1 and 50"));
    return;
  }
  ...
  res.status(200).json(
    buildPaginatedResponse(result.services, "Services retrieved successfully", {
...
export const getServiceByIdHandler = async (
  ...
  if (!uuidRegex.test(id)) {
    res.status(400).json(buildErrorResponse("Invalid service ID format"));
    return;
  }
  ...
  if (!service) {
    res.status(404).json(buildErrorResponse("Service not found"));
    return;
  }

  res
    .status(200)
    .json(buildSuccessResponse(service, "Service retrieved successfully"));
};
...
export const updateServiceHandler = async (
  ...
  res
    .status(200)
    .json(buildSuccessResponse(updatedService, "Service updated successfully"));
};
...
export const deleteServiceHandler = async (
  ...
  res
    .status(200)
    .json(buildSuccessResponseWithoutData("Service deleted successfully"));
};
