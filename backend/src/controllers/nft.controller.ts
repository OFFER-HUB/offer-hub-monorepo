...
export const registerMintedNFTHandler = async (
  ...
  if (!uuidRegex.test(user_id)) {
    res.status(400).json(buildErrorResponse("Invalid user ID format"));
    return;
  }
  ...
  res
    .status(201)
    .json(buildSuccessResponse(newNFT, "NFT minting recorded successfully"));
};
...
export const getNFTsByUserHandler = async (
  ...
  res
    .status(200)
    .json(buildSuccessResponse(nfts, "User NFTs retrieved successfully"));
};
...
export const getNFTByIdHandler = async (
  ...
  const uuidRegex =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(id)) {
    res.status(400).json(buildErrorResponse("Invalid NFT ID format"));
    return;
  }
  ...
  if (!nft) {
    res.status(404).json(buildErrorResponse("NFT not found"));
    return;
  }

  res.status(200).json(buildSuccessResponse(nft, "NFT retrieved successfully"));
};
