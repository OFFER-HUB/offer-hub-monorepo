  ...
  try {
    ...
    if (!service_id || !client_id || !message) {
      return res
        ...
        .json(
          buildErrorResponse(
            "Missing required fields: service_id, client_id, message",
          ),
        );
    }
    ...
    if (typeof message !== "string" || message.trim().length === 0) {
      return res
        .status(400)
        .json(buildErrorResponse("Message must be a non-empty string"));
    }
    ...
    res
      .status(500)
      .json(buildErrorResponse("Failed to create service request"));
  }
};
...
export const getRequestsForFreelancerHandler = async (
  ...
  try {
    ...
    if (!uuidRegex.test(freelancerId)) {
      return res
        .status(400)
        .json(buildErrorResponse("Invalid UUID format for freelancerId"));
    }

    const requests =
      await serviceRequestService.getRequestsForFreelancer(freelancerId);
    ...
    res
      .status(500)
      .json(buildErrorResponse("Failed to retrieve service requests"));
  }
};
...
export const updateRequestStatusHandler = async (
  ...
  try {
    ...
    if (!uuidRegex.test(id) || !uuidRegex.test(freelancerId)) {
      return res
        .status(400)
        .json(buildErrorResponse("Invalid UUID format for id or freelancerId"));
    }
    ...
    res.status(500).json(buildErrorResponse("Failed to update request status"));
  }
};
