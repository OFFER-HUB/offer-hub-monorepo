  ...
  try {
    ...
    if (!project_id || !freelancer_id || !message) {
      return res
        ...
        .json(
          buildErrorResponse(
            "Missing required fields: project_id, freelancer_id, message",
          ),
        );
    }
...
export const getApplicationsByProjectHandler = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const updateApplicationStatusHandler = async (
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json(buildErrorResponse("Application ID is required"));
    }
    ...
    res
      .status(500)
      .json(buildErrorResponse("Failed to update application status"));
  }
};
