...
describe("Validation Utilities", () => {
  ...
  describe("validateWalletAddress", () => {
    it("should validate Ethereum addresses", () => {
      expect(
        validateWalletAddress("0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6"),
      ).toBe(true);
      expect(
        validateWalletAddress("0x742d35cc6634c0532925a3b8d4c9db96c4b4d8b6"),
      ).toBe(true);
  ...
  describe("validateMonetaryAmount", () => {
    it("should validate correct monetary amounts", () => {
      ...
      expect(validateMonetaryAmount(100.5)).toBe(true);
  ...
  describe("validateObject", () => {
    describe("CONTRACT_CREATION_SCHEMA", () => {
      ...
      it("should reject invalid contract data", () => {
        ...
        expect(result.errors.some((e) => e.field === "contract_type")).toBe(
          true,
        );
        expect(result.errors.some((e) => e.field === "freelancer_id")).toBe(
          true,
        );
        expect(
          result.errors.some((e) => e.field === "contract_on_chain_id"),
        ).toBe(true);
        expect(result.errors.some((e) => e.field === "amount_locked")).toBe(
          true,
        );
      });
      ...
      it("should handle missing required fields", () => {
        ...
        const result = validateObject(contractData, CONTRACT_CREATION_SCHEMA);
        expect(result.isValid).toBe(false);
        expect(result.errors.some((e) => e.code === "REQUIRED_FIELD")).toBe(
          true,
        );
      });
    });
