...
await sdk.orders.release(order.id, 'Buyer approved release');
```
...
try {
  ...
  if (error.code === 'FUNDING_TIMEOUT') {
    ...
    if (order.status === 'ESCROW_FUNDING') {
      // Transaction pending — wait for the order.escrow_funded SSE event or poll
...
## Real-Time Escrow Events (SSE)

Escrow lifecycle events are delivered over the [SSE event stream](/docs/api-reference/webhooks) at `GET /api/v1/events` — there is no outbound webhook API:

```bash
curl -N "http://localhost:4000/api/v1/events?types=order.escrow_creating,order.escrow_funded,order.released,order.refunded" \
  -H "Authorization: Bearer ohk_live_your_api_key"
```
...
events.on('order.escrow_funded', (data) => {
  console.log('Escrow funded:', data.fundedAt);
});

events.on('order.released', (data) => {
  console.log('Released to seller:', data.amount);
});
```

See [Real-Time Events & Webhooks](/docs/api-reference/webhooks) for the full event catalog, connection parameters, and replay support.

## Security Considerations

1. **Never expose private keys** — All signing happens server-side
2. **Use the SSE stream** — Real-time updates arrive over `GET /api/v1/events`; there is no outbound webhook API
