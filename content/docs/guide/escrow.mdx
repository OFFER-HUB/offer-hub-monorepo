...
Escrow lifecycle events are delivered over the [SSE event stream](/docs/api-reference/events) at `GET /api/v1/events` — there is no outbound webhook API:
...
See [Events (SSE)](/docs/api-reference/events) for the full event catalog, connection parameters, and replay support.
