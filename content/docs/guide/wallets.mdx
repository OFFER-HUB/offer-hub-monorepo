---
title: Wallets Guide
description: Understanding invisible wallets in OFFER-HUB — automatic Stellar wallet creation and management.
order: 5
section: Guides
---
...
```mermaid
flowchart LR
    subgraph UserSees["User sees"]
        Balance["Balance: $100.00 USD"]
    end
    subgraph BehindTheScenes["What actually happens"]
        Wallet["Stellar Wallet: GCV24WNJ..."]
        USDC["USDC Balance: 100.0000000"]
        Key["Private Key: encrypted in DB"]
    end
    Balance --> BehindTheScenes
```
...
Balance events are delivered over the [SSE event stream](/docs/api-reference/webhooks) at `GET /api/v1/events` — there is no outbound webhook API:
