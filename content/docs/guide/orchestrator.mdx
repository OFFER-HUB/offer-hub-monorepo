...
```mermaid
flowchart TB
    subgraph Marketplace["Your Marketplace"]
        Frontend[Frontend]
        Backend[Backend]
        Frontend --> Backend
    end

    Backend -->|REST API| Orchestrator

    subgraph Orchestrator["OFFER-HUB Orchestrator"]
        Ledger[Balance Ledger]
        Escrow[Escrow Engine State Machine]
        SSE[SSE Event Stream]
        Idemp[Idempotency Key Store]
    end

    Orchestrator --> Stellar
    Orchestrator --> Trustless

    Stellar[Stellar Network USDC]
    Trustless[Trustless Work Soroban Contracts Non-custodial]
```
