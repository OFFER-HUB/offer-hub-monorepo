...
```mermaid
sequenceDiagram
    participant User as User's External Wallet
    participant OH as OFFER-HUB

    User->>OH: 1. Send USDC
    Note over OH: 2. Detect on blockchain
    Note over OH: 3. Update balance
    OH->>User: 4. Emit event
```
...
<Callout type="note">
  SSE is the **only** real-time event mechanism — there is no outbound webhook subscription API. See [Real-Time Events & Webhooks](/docs/api-reference/webhooks) for the full event catalog and connection details.
</Callout>
