...
`mermaid
sequenceDiagram
    accTitle: Dispute open and resolve
    accDescr: Buyer opens dispute, escrow frozen, platform reviews, resolves on-chain with release or refund.
    autonumber
    participant Buyer
    participant Orchestrator
    participant Support as Platform Support
    participant TW as Trustless Work Contract
    Buyer->>Orchestrator: POST /orders/{id}/resolution/dispute
    Orchestrator->>TW: disputeEscrow (buyer signs)
    Orchestrator-->>Buyer: 200 DISPUTED
    Support->>Orchestrator: GET /disputes/{id}
    alt FULL_RELEASE
        Support->>Orchestrator: POST /disputes/{id}/resolve
        Orchestrator->>TW: resolveDispute(100% seller)
    else FULL_REFUND
        Support->>Orchestrator: POST /disputes/{id}/resolve
        Orchestrator->>TW: resolveDispute(100% buyer)
    end
    Orchestrator-->>Buyer: SSE order.released or order.refunded
`
...
4. **Both parties notified** — Buyer and seller are notified (via your app's SSE integration)
...
// Open a dispute
const dispute = await sdk.disputes.open('ord_xyz789', {
...
## Real-Time Dispute Events (SSE)

Dispute lifecycle events are delivered over the [SSE event stream](/docs/api-reference/events) at `GET /api/v1/events` — there is no outbound webhook API:

```bash
curl -N "http://localhost:4000/api/v1/events?types=order.disputed,dispute.opened,dispute.resolved" \
  -H "Authorization: Bearer ohk_live_your_api_key"
```
...
The `order.disputed` event payload looks like this:
...
When a dispute is resolved, the escrow settles and the corresponding order events (`order.released` or `order.refunded`) fire on the same stream.
