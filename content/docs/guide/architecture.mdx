...
Each marketplace owns an isolated Orchestrator and database.

This deployment boundary is verified against the Orchestrator's `apps/api/src/main.ts` bootstrap and `packages/database/prisma/schema.prisma`; each installation supplies its own service configuration and database connection.
