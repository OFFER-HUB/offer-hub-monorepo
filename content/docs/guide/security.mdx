...
1. **Authenticate every connection** — SSE requires a valid API key in the `Authorization` header. Never expose the stream unauthenticated.
...
```typescript
const idempotentClient = offerHubClient.withIdempotencyKey(`release:${orderId}:${buyerId}`);
...
- [Events (SSE)](/docs/api-reference/events) — SSE event stream and event catalog
