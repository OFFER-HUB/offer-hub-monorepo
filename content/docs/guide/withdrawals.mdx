...
```mermaid
flowchart LR
    A[Balance debited] --> B[Transaction signed]
    B --> C[Submitted]
    C --> D[Confirmed]
    D --> E[Complete]
```
...
There is no outbound webhook API — withdrawal events are delivered over the [SSE event stream](/docs/api-reference/webhooks) at `GET /api/v1/events`:
