...
await sdk.orders.createEscrow('ord_xyz789');
...
```ts
await sdk.orders.fundEscrow('ord_xyz789');
...
```mermaid
flowchart LR
    subgraph O["One order"]
      A1[Escrow funded] --> B1[Milestone delivered]
      B1 --> C1[Milestone completed]
      C1 --> D1[Next milestone]
    end
```
...
```ts
await sdk.orders.release('ord_xyz789', 'Buyer approved release');
```
...
```ts
await sdk.disputes.open('ord_xyz789', {
...
Lifecycle events are delivered over the [SSE event stream](/docs/api-reference/events) at `GET /api/v1/events` — there is no outbound webhook API. Subscribe to keep your application in sync:
