...
The OFFER-HUB deployment consists of the API, its shared infrastructure, and the Stellar network:
...
The API registers and processes BullMQ jobs in the same service. The separate `apps/worker` package
is deprecated and should not be started as an additional process.

| Queue | Purpose |
|-------|---------|
| `webhooks` | Process Airtm and Trustless Work webhooks |
| `reconciliation` | Sync topups, withdrawals, escrows, and missed deposits |
| `notifications` | Send email and webhook notifications |
| `dead-letter-queue` | Retain jobs that exhausted their retries |
...
# Check the webhook queue length
docker compose exec redis redis-cli llen "bull:webhooks:wait"
```

Start the API; it also processes background jobs:
