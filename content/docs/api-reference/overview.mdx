...
OFFER-HUB does **not** provide an outbound webhook subscription API. The real-time integration path is the **SSE event stream** at `GET /api/v1/events`.

See the [Real-Time Events & Webhooks documentation](/docs/api-reference/webhooks) for:
- Connecting to the SSE stream and replaying missed events
- The full event catalog
- The inbound provider webhook receivers (self-hosting only)
