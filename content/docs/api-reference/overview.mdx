...
Subscribers consume real-time domain events over the [SSE event stream](/docs/api-reference/events) at `GET /api/v1/events`. Separately, the Orchestrator receives inbound webhooks from external providers (Airtm, Trustless Work) to process payment and escrow lifecycle events — see the [Inbound Webhooks](/docs/api-reference/webhooks) reference for signature verification and deduplication behavior.
...
[orchestrator `IdempotencyGuard`](https://github.com/OFFER-HUB/OFFER-HUB/blob/main/apps/api/src/common/guards/idempotency.guard.ts)
and
[`IdempotencyInterceptor`](https://github.com/OFFER-HUB/OFFER-HUB/blob/main/apps/api/src/common/interceptors/idempotency.interceptor.ts).
...
1. **Invalid key**: Returns `400 VALIDATION_ERROR`; the key must be a UUID v4.
2. **First request**: Processed normally and stored for replay.
3. **Repeated request with the same key and body**: Returns the original response
   and sets `Idempotency-Replay: true`.
4. **Different body with the same key**: Returns `409 IDEMPOTENCY_KEY_REUSED`.
5. **Original request still processing**: Returns `409
   IDEMPOTENCY_KEY_IN_PROGRESS`.

Example for a reused key:
...
Keys are retained for 24 hours. Reuse after that period is treated as a new
request.
...
[`RateLimitGuard`](https://github.com/OFFER-HUB/OFFER-HUB/blob/main/apps/api/src/common/guards/rate-limit.guard.ts).
...
Wallet endpoints are scoped to a user: `/users/{userId}/wallet`. Full reference in [Wallet & Top-ups](/docs/api-reference/wallet-topups).

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/users/{userId}/wallet` | <Badge variant="success">GET</Badge> | [Get wallet info](/docs/api-reference/wallet-topups#get-users-userid-wallet) |
| `/users/{userId}/wallet/deposit` | <Badge variant="success">GET</Badge> | [Get deposit address](/docs/api-reference/wallet-topups#get-users-userid-wallet-deposit) |
| `/users/{userId}/wallet/transactions` | <Badge variant="success">GET</Badge> | [Transaction history](/docs/api-reference/wallet-topups#get-users-userid-wallet-transactions) |

### Top-ups

Full reference with request/response schemas in [Wallet & Top-ups](/docs/api-reference/wallet-topups).

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/topups` | <Badge variant="primary">POST</Badge> | [Create top-up](/docs/api-reference/wallet-topups#post-topups) |
| `/topups` | <Badge variant="success">GET</Badge> | [List top-ups](/docs/api-reference/wallet-topups#get-topups) |
| `/topups/{id}` | <Badge variant="success">GET</Badge> | [Get top-up](/docs/api-reference/wallet-topups#get-topups-id) |
| `/topups/{id}/refresh` | <Badge variant="primary">POST</Badge> | [Refresh status from Airtm](/docs/api-reference/wallet-topups#post-topups-id-refresh) |
| `/topups/{id}/cancel` | <Badge variant="primary">POST</Badge> | [Cancel top-up](/docs/api-reference/wallet-topups#post-topups-id-cancel) |
| `/topups/{id}/callback` | <Badge variant="success">GET</Badge> | [Airtm callback (public)](/docs/api-reference/wallet-topups#get-topups-id-callback) |
...
- [Events (SSE)](/docs/api-reference/events) - Real-time event stream and event catalog
- [Inbound Webhooks](/docs/api-reference/webhooks) - AirTM and Trustless Work receivers
- [Wallet & Top-ups](/docs/api-reference/wallet-topups) - Per-endpoint schemas for deposits and AirTM pay-ins
